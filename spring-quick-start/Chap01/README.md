...
**@Autowired**는 생성자나 메소드, 멤버변수 위에 모두 사용할 수 있다. 해당 타입의 객체를 찾아서 자동으로 할당한다.  
**@Qualifier**는 할당될 수 있는 객체가 2개 이상일 때, 이를 지정하기 위하여 사용할 수 있다.  
**@Resource**는 객체의 이름을 이용하여 할당한다. `@Component`로 지정된 이름을 그대로 사용하여 `@Resource(name="이름")` 꼴로 멤버 변수 위에서 사용할 수 있다.  
스프링 컨테이너가 생성하는 객체를 XML 설정으로 처리할 수도 있고, 어노테이션으로 처리할 수도 있다. 하지만 XML 설정만 사용하기에는 bean 등록을 많이 해야하고 의존관계 설정을 일일이 해야한다. 그렇다고 어노테이션만 설정하자니 자바 소스를 수정해야한다. 따라서 변경되지 않느 객체는 어노테이션으로 설정하여 사용하고, 변경될 가능성이 있는 객체는 XML 설정으로 사용하여 **적절히 병행**한다.
